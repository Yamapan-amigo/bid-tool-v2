...
def _extract_grade(text: str) -> tuple[str, bool | None]:
    ...
    # 地方自治体施行令第167条参照（全角・半角対応）
    if re.search(r"施行令.{0,5}第\s*(?:167|１６７)\s*条", text):
        return "地方自治体基準（施行令167条）", None

    # 広義の地方自治体独自資格パターン（PDFから取得したテキストで頻出）
    # A-D等級パターンが一切マッチしなかった場合にのみ到達するためフォールスポジティブは最小
    if re.search(
        r"入札参加資格(?:者)?名簿|入札参加資格(?:要件|要綱|審査)"
        r"|(?:一般|条件付)?競争入札参加資格(?:等)?(?:の確認|を得|審査|認定|に関する)"
        r"|入札参加資格(?:確認申請|審査要領)",
        text,
    ):
        return "地方自治体独自名簿", None
